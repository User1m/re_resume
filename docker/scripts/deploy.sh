...
APP_NAME=claudiusresume #claudiusmbemba

az webapp create --resource-group $RG \
--plan $APP_SERVICE_PLAN --name $APP_NAME --deployment-container-image-name $DOCKER_IMAGE
...
az webapp deployment container config --name $APP_NAME --resource-group $RG --enable-cd true
